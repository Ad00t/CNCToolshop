...
namespace CNCToolshop {
    class ImageToGcode {
        ...
        public ImageToGcode(double width, double height, double lineStep, double freezone, double feed) {
            _Width = width;
            _Height = height;
            _LineStep = lineStep;
            _Freezone = freezone;
        ...
        public string Generate(Bitmap image) {
            ...
            string gCode = "";
            gCode += string.Format(nfi, "F{0:0.###}", _Feed);
            for (int y = 0; y < linesCount; y++)    //Шагаем по строкам
            {
                ...
                gCode += string.Format(nfi, fastMove, firstDarkX * pixelWidth - _Freezone, -y * _LineStep, 0.0);
                ...
                for (int x = firstDarkX; x < iWidth; x++) {
                    ...
                    gCode += string.Format(nfi, feedMove, lastX, lastLaserState);
                ...
                if (lastLaserState > 0.0) {
                    lastX = _Width;
                    gCode += string.Format(nfi, feedMove, _Width, lastLaserState); //Завершаем строку и выключаем лазер
                }

                if (_Freezone > 0.0)
                    gCode += string.Format(nfi, feedMove, lastX + _Freezone, 0); //Холостой ход для торможения
